...
export class GisLineGeometry extends GeometryBase {

    constructor(segmentCount: number) {
        super();
        this.buildGeometry(segmentCount);
    }

    //0 1
    //2 3
    private buildGeometry(segmentCount: number): void {
        let indices_arr: Uint32Array = new Uint32Array(segmentCount * 6);
        let offset = 0;
        let vOffset = 0;
        ...
        let v_index = new Float32Array(segmentCount * 4);
        for (let i = 0; i < segmentCount; i++) {
            offset = i * 12;
